...
| 🚧 **`enforce-worktree-boundary-bash.sh`** | `PreToolUse` on `Bash` | Bash-side companion to the edit-boundary hook. Blocks a shell command that `cd`/`pushd`es into a different git worktree than the session's cwd — closing the trap where a worktree agent runs `cd /path/to/main && npx prettier --write web/…` so the mutating command lands in the main checkout instead of the worktree edits (the edit-tool hook never sees it because it's a `Bash` call). Detects the escape mechanism, not a per-tool allowlist, so nothing rots; reads, `/tmp`, and non-repo dirs are allowed. Not caught: a bare absolute path with no `cd` (e.g. `prettier --write /main/web/foo.ts`). Verified by `hooks/test-enforce-worktree-boundary-bash.sh`. Bypass with `--dangerously-skip-permissions` or by removing the hook from `~/.claude/settings.json`. |
