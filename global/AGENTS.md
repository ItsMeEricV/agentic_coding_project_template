...
- **NEVER create a GitHub issue without explicit user permission. HARD RULE.** Opening an issue adds a row to the user's backlog that has to be triaged, closed, or lived with. Past agents have unprompted-created issues while writing PR bodies, audit reports, and follow-up plans — the result is backlog bloat and confusion about what was actually decided. Before creating an issue by any means, propose title + one-paragraph summary and wait for explicit "yes, open it." Editing/commenting on an existing issue, or closing one the user told you to close, is fine without re-asking.
